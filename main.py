...
def initialization_ui():
    ...
    try:
        ...
        print(f"[{debug_get_time()}] ERROR: initialization_ui\n{e}")
...
def clear_screen():
    ...
    try:
        ...
        print(f"[{debug_get_time()}] ERROR: clear_screen\n{e}")
...
def debug_get_time():
    ...
    try:
        ...
        print(f"[{debug_get_time()}] ERROR: debug_get_time\n{e}")
        return "FAIL-TO-GET-TIME"
...
def resource_path(relative_path):
    ...
    try:
        ...
        print(f"[{debug_get_time()}] ERROR: resource_path\n{e}")
...
def page_lang():
    ...
    try:
        ...
        value = tk.StringVar()
        value.set(lang_get[f"lang_{lang_current}"])

        menu = tk.OptionMenu(ui, value, *languages, command=lambda event: change_lang(event))
        menu.config(font=("Microsoft YaHei",20), width=18)
        menu.pack()
        
        tk.Button(ui, text=lang_get["common_btn_return"], font=("Microsoft YaHei",14), width=16, command=page_mode).pack(side="top", pady=10, padx=20)
    except Exception as e:
        print(f"[{debug_get_time()}] ERROR: page_lang\n{e}")
...
def MAIN():
    ...
    try:
        ...
        print(f"[{debug_get_time()}] ERROR: MAIN\n{e}")
